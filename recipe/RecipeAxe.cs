namespace jfcraft.recipe
{
	...
	public class RecipeAxe : Recipe
	{
	  ...
	  public override Item make(Item[] items)
	  {
		...
		if (id == Items.DIAMOND)
		{
			return new Item(Items.DIAMOND_AXE, 1.0f);
		}
		if (id == Items.GOLD_INGOT)
		{
			return new Item(Items.GOLD_AXE, 1.0f);
		}
		if (id == Items.IRON_INGOT)
		{
			return new Item(Items.IRON_AXE, 1.0f);
		}
		if (id == Blocks.COBBLESTONE)
		{
			return new Item(Items.STONE_AXE, 1.0f);
		}
		if (id == Blocks.PLANKS)
		{
			return new Item(Items.WOOD_AXE, 1.0f);
		}
		return null;
	  }
	}
