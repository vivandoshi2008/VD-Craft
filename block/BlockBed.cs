namespace jfcraft.block
{
	...
	public class BlockBed : BlockBase
	{
	  private static GLModel model_head, model_feet;
	  private static int VAR_FEET = 0;
	  private static int VAR_HEAD = 1;
	  public BlockBed(string name, string[] names, string[] images) : base(name, names, images)
	  {
		isOpaque = false;
		isAlpha = false;
		isDir = true;
		...
		addBox(0, 0, 0, 15, 7, 15,BlockHitTest_Type.BOTH);
	  ...
	  public override bool place(Client client, Coords c)
	  {
		...
		if (c2.chunk == null)
		{
			Static.log("BlockBed.place():Error:next chunk not found");
			return false;
		}
		bool placed = c1.chunk.setBlocksIfEmpty(c1, id, bits1, c2, id, bits2);
		if (!placed)
		{
			return false;
		}
