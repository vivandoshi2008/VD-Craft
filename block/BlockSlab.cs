...
namespace jfcraft.block
{
	...
	public class BlockSlab : BlockBase
	{
	  private static GLModel model;
	  public BlockSlab(string id, string[] names, string[] images) : base(id, names, images)
	  {
		isOpaque = false;
		isAlpha = false;
		isComplex = true;
	  ...
	  //dir bits
	  private const int AB = 0x1;
	  private const int BB = 0x2;
	  ...
	  public override bool canPlace(Coords c)
	  {
		...
		if (block1.id == id)
		{
		  ...
		  if (y >= 0.5f)
		  {
			...
			if ((dir & AB) != 0)
			{
				return false; //already have upper
			}
		  }
		  else
		  {
			...
			if ((dir & BB) != 0)
			{
				return false; //already have lower
			}
		  }
		  return true;
		}
